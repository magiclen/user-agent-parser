...
use crate::rocket::request::{Request, FromRequest, Outcome as OutcomeResult};
use crate::rocket::{State, Outcome};
...
impl<'a, 'r> FromRequest<'a, 'r> for UserAgent<'a> {
    ...
    fn from_request(request: &'a Request<'r>) -> OutcomeResult<Self, Self::Error> {
        Outcome::Success(
            {
                let user_agent: Option<Cow<'a, str>> = request.headers().get("user-agent").next().map(|s| Cow::from(s));
        ...
        Outcome::Success(
            {
                ...
                match user_agent {
                    Some(user_agent) => {
                        user_agent_parser.inner().parse_product(user_agent)
                    }
                    None => {
                        Product::default()
                    }
        ...
        Outcome::Success(
            {
                ...
                match user_agent {
                    Some(user_agent) => {
                        user_agent_parser.inner().parse_os(user_agent)
                    }
                    None => {
                        OS::default()
                    }
        ...
        Outcome::Success(
            {
                ...
                match user_agent {
                    Some(user_agent) => {
                        user_agent_parser.inner().parse_device(user_agent)
                    }
                    None => {
                        Device::default()
                    }
        ...
        Outcome::Success(
            {
                ...
                match user_agent {
                    Some(user_agent) => {
                        user_agent_parser.inner().parse_cpu(user_agent)
                    }
                    None => {
                        CPU::default()
                    }
        ...
        Outcome::Success(
            {
                ...
                match user_agent {
                    Some(user_agent) => {
                        user_agent_parser.inner().parse_engine(user_agent)
                    }
                    None => {
                        Engine::default()
                    }
